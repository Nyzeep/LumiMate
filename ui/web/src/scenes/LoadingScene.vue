  ...
  <section class="scene-panel scene-panel--loading" :class="{ 'is-active': active }" aria-label="加载空间">
    ...
    <div class="scene-grid">
      ...
      <div class="span-4 loading-core">
        <OrbitLoading :progress="progressRatio" :loaded="state.runtime.loaded" :caption="state.runtime.progressMessage" label="加载轨道" />
      </div>

      <div class="span-4 scene-side-stack">
        <HoloCard class="loading-readout-card" tone="strong">
          <p class="scene-kicker">当前核心</p>
          <h2 class="metric-display">{{ currentModelName }}</h2>
          <MetricLine label="整体进度" :value="`${progressPercent}%`" :progress="progressRatio" />
          <MetricLine label="当前状态" :value="stateLabel" :progress="progressRatio" />
        </HoloCard>
      </div>

      <div class="span-12">
        <HoloCard class="step-card">
          <div v-for="step in loadingSteps" :key="step.label" class="step-row" :class="{ 'is-active': step.active, 'is-done': step.done }">
